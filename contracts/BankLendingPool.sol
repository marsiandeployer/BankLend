...
/// @title BankLendingPool
/// @notice Simplified lending pool with bank-mode admin withdrawal
/// @dev Uses onout.org Storage contract for admin auth (same as launchpad/dex)
contract BankLendingPool is ReentrancyGuard {
    ...
    uint256 public constant MAX_LTV = 90; // max 90% LTV for safety
    uint256 public constant LIQUIDATION_THRESHOLD = 80; // liquidate if health < 1.0 at 80% threshold
    ...
    // Deposit tracking (ERC4626-inspired shares)
    uint256 public totalDeposited;  // total USDT deposited (including accrued interest)
    ...
    struct Borrow {
        ...
        uint256 startTime;        // last interest accrual timestamp
    }
    mapping(address => Borrow) public borrows;
    uint256 public totalBorrowed;  // sum of all principal + interestOwed

    // Bank mode
    uint256 public adminWithdrawn; // how much admin pulled from pool

    // Interest rates (basis points, 1 bp = 0.01%)
    uint256 public depositAPY;  // e.g. 800 = 8%
    uint256 public borrowAPY;   // e.g. 1500 = 15%
    uint256 public lastAccrualTime;

    // Collateral config
    struct CollateralConfig {
        address priceFeed;  // Chainlink feed
        uint256 ltv;        // loan-to-value in %, e.g. 75
        bool enabled;
    }
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == STORAGE.getData(domain).owner,
            "BankLend: not admin"
        );
        _;
    }
    ...
    /// @notice Add or update a supported collateral token
    ...
    /// @notice Disable a collateral token (existing borrows not affected)
    ...
    /// @notice Update interest rates (basis points, 100 = 1%)
    ...
    /// @notice Bank mode: admin withdraws from pool
    /// @dev Funds leave the contract but totalDeposited unchanged — utilization rises
    function adminWithdraw(uint256 amount) external onlyAdmin nonReentrant {
        require(amount > 0, "BankLend: zero amount");
        uint256 available = availableLiquidity();
        require(amount <= available, "BankLend: insufficient liquidity");
    ...
    /// @notice Bank mode: return funds to pool (anyone can call to restore liquidity)
    ...
    /// @notice Deposit USDT, receive shares
    ...
    /// @notice Withdraw USDT by burning shares
    ...
    /// @notice Borrow USDT against collateral
    function borrow(
        address collateralToken,
        uint256 collateralAmount,
    ...
    /// @notice Repay loan and receive collateral back
    ...
    function liquidate(address borrower) external nonReentrant {
        ...
        uint256 contractCollateralBalance = IERC20(b.collateralToken).balanceOf(address(this));
        if (seizedCollateral > contractCollateralBalance) {
            seizedCollateral = contractCollateralBalance;
        }
    ...
    /// @notice Get admin address from Storage
    function admin() external view returns (address) {
        return STORAGE.getData(domain).owner;
    }
    ...
    /// @dev Global interest accrual for depositors
    function _accrueInterest() internal {
        ...
        uint256 interestEarned = (totalBorrowed * borrowAPY * elapsed) /
            (SECONDS_PER_YEAR * 10000);

        // Add interest to totalDeposited so share value increases
        totalDeposited += interestEarned;
        totalBorrowed += interestEarned;
        lastAccrualTime = block.timestamp;
    }

    /// @dev Per-borrower interest accrual
    function _accrueBorrowerInterest(address borrower) internal {
        Borrow storage b = borrows[borrower];
        if (b.principal == 0) return;
        uint256 elapsed = block.timestamp - b.startTime;
        uint256 interest = (b.principal * borrowAPY * elapsed) /
            (SECONDS_PER_YEAR * 10000);
        b.interestOwed += interest;
        b.startTime = block.timestamp;
    }

    /// @dev Get collateral value in USDT (6 decimals, same as USDT)
    function _getCollateralUSD(address token, uint256 amount) internal view returns (uint256) {
        ...
        // Normalize: (amount * price) → USDT decimals (6)
        // amount has tokenDecimals, price has feedDecimals, result should have 6
        uint256 usdValue = (amount * uint256(price)) /
            (10 ** (tokenDecimals + feedDecimals - 6));

        return usdValue;
    }

    /// @dev Get ERC20 decimals with fallback
    ...
    /// @dev Check if borrower's position is healthy
    function _isHealthy(address borrower) internal view returns (bool) {
        return _healthFactor(borrower) >= PRECISION;
    }

    /// @dev Compute health factor: collateralUSD * liquidationThreshold / totalOwed
...
/// @dev Minimal interface to get token decimals
